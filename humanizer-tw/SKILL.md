...
version: "1.3.1"
description: |
  去除中文文字的 AI 生成痕跡（時代開場白、連接詞濫用、互聯網黑話、翻譯腔、結尾套話），並把簡轉繁殘留與中國用語在地化成台灣寫法。
  使用時機：使用者貼一段像 AI 寫的中文要潤色、去機器味；校對 OpenCC 簡轉繁或從簡體翻譯來的文件；要求「只標問題不改」的診斷；提供自己的文章樣本要求對齊文風。純節奏琢磨交給 good-writing-tw，英文交給 humanizer-en。
  觸發詞：/humanizer-tw、去 AI 味、去機器味、簡轉繁校對、OpenCC 漏網、在地化、這段哪裡像 AI。
user-invocable: true
argument-hint: "<article text or file path to humanize>"
---
...
- 觸發詞：/humanizer-tw、humanize（中文文本）、去除 AI 痕跡、人性化、去機器味、簡轉繁校對、OpenCC 漏網、簡中翻譯校對、在地化、標註模式、只標問題不改、這段哪裡像 AI、先標問題不改寫、個人風格校準、風格校準、語氣對齊、用我的文風、voice matching。使用者沒說「humanize」也該主動用：貼上像 AI 寫的中文要潤色改順、校對 OpenCC s2twp 轉過的繁中或從簡體 fork 來的 README 與技術文件、想清掉翻譯腔在地化成台灣用語，都算。
- 標註模式：「只標問題不改」「這段哪裡像 AI」「先別改，先診斷」→ 只列問題，不輸出改寫稿（見模式三）。
- 個人風格校準：使用者提供自己的書寫樣本時，先分析樣本再改寫（見下一節）。
- 英文文本去 AI 味用 humanizer-en；本 skill 專責中文。
...
9. **句式結構 > 詞彙替換** — 一段話像不像 AI，骨架（對稱三元列舉、每段段末總結、三段式論證）往往比用詞更關鍵。命中這類結構性痕跡時，只抽換裡頭的黑話詞沒用，骨架沒動就還是一眼像 AI——這種情況要動句型結構本身。但這不是鼓勵到處重寫：沒有結構性痕跡的句子，仍照第 3、7 條保守處理。
...
### 類別五：公式化結構
開頭總結-中間三點-結尾金句、三段式論證、否定式排比（不僅...更是）、每段段末都自動補一句總結（「由此可見」「這說明了」），以及三項等長等重的對稱列舉。打破公式。
...
- **臆測填空**：查不到資料時不直說「查不到」，反而寫一段「資料很少」的話再用推測補滿——「為人低調，鮮少公開私人生活」「據信」「很可能出身於…」。改成直說「現有資料未記載」或整段刪掉；不替空白編故事，猜測不可包裝成事實。
...
- **短句連發戲劇腔**：連續多個短促斷句堆疊造勢——「然後它來了。沒有猶豫。沒有退路。」單一短句強調是正常寫作；連發才是製造出來的戲劇感，併回完整句。
- **金句公式**：「X 是 Y 的 Z」「X 成了陷阱」「X 不是工具，而是鏡子」——把普通論點包裝成可轉發警句，聽起來深刻卻沒增加精確度。改回它想說的具體主張。**位於全文結尾的金句一律刪除、不得改寫成一句更漂亮的金句**——用文中已有的最具體一句收尾；真需要收束就補一句平實的下一步或結論。
- **假坦率開場**：「說實話？」「講白了，」「這麼說吧」當獨立 hook、先停頓再揭曉的表演式坦率。真正坦率的人直接把話說出來。刪掉開場，直接講。（句中自然出現的「說實話」是日常口語，不算。）
- **多數人迷思開場**：「大多數人都搞錯了」「沒有人告訴你的是」「這是 90% 的人忽略的」——把作者捧成唯一清醒者的空殼開場，後面往往接一個平凡論點。刪開場，讓主張自己站住。（真有論據的反主流論點不算——tell 是宣稱眾人皆錯卻拿不出對應證據。）
- **冒號揭曉**：「最妙的是：它會自己學。」「關鍵在於：不是技術，是人。」名詞短語＋冒號＋戲劇性揭曉的句型。改寫成平實句子；冒號留給列表、定義、標籤、引言。
- **自問自答**：「為什麼會這樣？因為……」「你以為是 X 嗎？錯了，是 Y。」自設問題再自己揭曉的表演結構。刪掉設問，直接講。（教學文／FAQ 的真設問是合法修辭，見防誤殺。）
...
**動手前先讀聲音**：待改文本若是「人寫的草稿沾染 AI 痕跡」（而非純 LLM 輸出），先從文本本身記下 3–5 個作者聲音信號——用詞層級、節奏、直率程度、幽默、猶豫、離題習慣——改寫時保留這些信號，只清 AI 痕跡。純 LLM 輸出讀不出個人聲音時此步從簡，逕用文體判斷。（與「個人風格校準」的差別：那是使用者額外提供樣本；這是從待改稿本身讀，不需樣本。）
...
- **改得太平整？** 人寫的東西本來有起伏——有的段落工整、有的鬆散，句子有長有短。若改完每段都是同一種乾淨語氣、同一個節奏，那本身又成了新的 AI tell。不必把每一處輕微痕跡都抹平：每千字容許留 2-3 處無害的小東西（一個兩項並列、一個輕度過渡詞），刻意留點不平整反而更像人。但高權重套話（客套結尾、無源歸因、時代開場）仍照刪，不在留白之列。
...
**不斷言作者身分**：標註列出的是 pattern 證據（使用者可自行核對的具體觸發點），**不是「這段是 AI 寫的」的判定**——人也會寫出這些 pattern（見 [references/detection-guidance-zh.md](references/detection-guidance-zh.md)），判定作者身分不是本 skill 的工作。使用者直接問「這是不是 AI 寫的」時，回答 pattern 密度與證據，明說無法斷言作者。
...
- ✓ 沒有公式化列舉或逐段段末總結？（「首先…其次…」等長對稱 → 破成篇幅隨分量；每段「由此可見」式收尾 → 刪或改過渡）
- ✓ 沒有「讓我們拭目以待」等結尾套話
...
- ✓ 沒有短句連發戲劇腔、金句公式、假坦率開場？（連續斷句造勢、「X 是 Y 的 Z」、獨立成句的「說實話？」）；結尾金句是刪除、不是改寫成更好的金句？
- ✓ 沒有多數人迷思開場、冒號揭曉、自問自答？（「沒有人告訴你」「最妙的是：」「為什麼？因為……」）
- ✓ 沒有臆測填空？（「為人低調」「據信」式的猜測包裝——查不到就直說查不到）
- ✓ 判定有先看「徵兆叢集」而非孤立單點？（避免誤殺正常人類書寫）
- ✓ 沒改得太平整？（每千字留 2-3 處無害小痕跡，未過度均質化成同一語氣／節奏）
