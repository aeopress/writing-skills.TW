...
**拆句 ≠ 連發造勢（cross-guard）**：拆長句的目的是因果清晰與節奏，不是製造戲劇感。連續三個以上短促斷句堆疊語勢（「沒有人停下。沒有人回頭。一切都變了。」）是新一代 AI 文的 tell（humanizer-tw 類別十三「短句連發戲劇腔」）。拆完的句子要長短相間：一兩個短句收束可以，整段拆成等短斷句即失敗——那不是節奏，是把一種 AI 味換成另一種。
...
節奏規則的數字（氣口 15–20 字、句長差 > 5 字、單句 30 字）是**預設值，不是鐵律**。硬套數字上限的失敗模式是把作者的風格也剷平——比漏改更糟：

1. **作者刻意的風格不算違規**：有內部停頓、念得出口的長句（余光中式綿長句）、刻意的排比、刻意的重複——能看出作者「為什麼這樣寫」的地方保留，不硬拆。判準同 humanizer-tw 的防誤殺：寧可漏改，不可錯改。
2. **別把全文改成同一種節奏**：每句都卡在 15–20 字、每句句長都精準差 5 字，改出來是節拍器不是文章——均質化本身就是新的 AI tell。規則用來抓「明顯單調」，不是把每句校準到公差內。
3. **風格樣本優先於預設數字**：使用者提供過自己的書寫樣本、或前一步 humanizer-tw 已做個人風格校準時，氣口／錯落／句尾的目標**對齊樣本**，不套本檔預設值。樣本作者愛寫長句，就別把成品全改成短句——串接的目的是保住使用者的聲音，不是覆蓋它。
...
8. **改過頭？** 拆句後沒有連續短促斷句造勢；作者刻意的長句／排比／重複沒被剷平；全文不是同一種節奏（見「防過度矯正」）
...
**輸出前回掃（兩頭都查）**：**改過頭？**——對照「防過度矯正」三條，把誤拆的風格句還原；**改不夠？**——對照檢查清單 1–7，補漏。
...
判斷原則：一個片段若**去掉中英文/數字/標點後剩不到 15 個中文字**，即使字元計數長也不算違規。
