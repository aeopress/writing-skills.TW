...
flowchart TD
    ...
    H -->|英文| EN["humanizer-en<br/>去 AI 味 · 37 patterns"]:::hum
    TW --> GW["good-writing-tw<br/>氣口 · 錯落 · 句尾 · 強動詞"]:::gw --> D["成稿"]:::done
    EN --> D
```

從 `humanizer` 路由入口進，自動判語言：**中文**走 `humanizer-tw` 去 AI 味，再接 `good-writing-tw` 琢磨節奏；**英文**走 `humanizer-en`（37 patterns）。`good-writing-tw` 是中文節奏琢磨、只在中文線，英文到 `humanizer-en` 即完成。
...
> 在當今這個 AI 飛速發展的時代，越來越多的內容創作者開始使用智能工具來提升視頻的質量。眾所周知，優質的內容能夠賦能個人品牌。本文將深入探討如何通過數據驅動的方式，全面優化你的創作流程。相信只要掌握這些技巧，你的帳號一定能夠實現質的飛躍。

**改寫後：**

> 很多內容創作者開始用 AI 工具來提升影片品質。好內容能幫個人品牌加分。這篇談怎麼用資料驅動的方式，把你的創作流程整個優化過一遍。掌握這幾個技巧，帳號的成長會很有感。

對照 humanizer-tw 抓到的問題（左欄原文 → 右欄實際改寫）：

| 病徵 | 原文 | 改寫後 |
|---|---|---|
| 時代開場 | 在當今這個 AI 飛速發展的時代 | （刪） |
| 中國用語滲入 | 視頻、質量、智能 | 影片、品質、AI |
| 互聯網黑話 | 賦能、質的飛躍 | 幫⋯⋯加分、成長很有感 |
| 共識／AI 高頻 | 眾所周知、深入探討、通過⋯⋯的方式 | 刪或換成白話 |
| 雞湯結尾 | 「相信只要⋯⋯你一定能夠⋯⋯」 | 換成平實陳述 |
| **不誤殺** | **優化**（合法台灣詞） | **保留**（沒改掉） |
...
| **[humanizer-en](humanizer-en/)** | 去 AI 味（英文） | 處理英文 AI 生成文字（37 patterns、voice calibration、annotate mode） | 基底 [blader/humanizer](https://github.com/blader/humanizer) v2.9.1，v3.0.0 起獨立演進 |
...
> **實測一句話：** 跟 5 支同類 skill 用同一份語料對打，去 AI 味 recall 大家都在 91–96%，但 humanizer-tw 的**誤殺率只有 8%、競品全在 29–42%**——招牌是「不把你寫對的台灣用語一起改掉」。見下方〈實測〉一節與[完整報告](docs/humanizer-tw-eval-2026-06.md)。
...
**13 類 AI 寫作痕跡**：開場白與連接詞、互聯網黑話、翻譯腔、書面語過重、公式化結構、結尾套話、語氣問題、中式 AI 句型、中國用語滲入、OpenCC 漏網、無源權威歸因、新世代 LLM 口癖（含臆測填空）、結構與修辭 tell（假深刻語／開場宣告／假範圍／diff-anchored／短句連發戲劇腔／金句公式／假坦率開場／多數人迷思開場／冒號揭曉／自問自答…）。
...
處理**英文** AI 生成文字的姊妹技能。基底 [blader/humanizer](https://github.com/blader/humanizer)（Siqi Chen，MIT）v2.9.1，**自 v3.0.0 起獨立演進**：上游三個缺口（faux-insight setups／colon reveals／self-answered questions，§34–36）已補，並自 humanizer-tw 回流結尾金句「刪除不改寫」紀律與 annotate mode（列 pattern 證據、不斷言作者身分）。上游此後以定期 diff＋逐條 cherry-pick 方式吸收，不整包合併。

- **37 條 pattern**：Wikipedia「Signs of AI writing」基底 33 條＋獨立擴充 4 條——inflated symbolism、promotional language、rule of three、em dash overuse、faux-insight setups、colon reveals、self-answered questions…
- **英文 gold 評測**：本地新 pattern 依 repo 紀律（規則＋guard＋gold 同次落地）以英文語料實測——v3.0.0 首輪 Fable 5 smoke：recall 100%、誤殺 0；v3.1.0 補 §9 appositive、§13 clipping guard、§37 contraction 各一正例一陷阱，共 13 筆，Fable 5 跑 recall 100%、誤殺 0，v3.0.0 四類無退步
...
>
> **中英自動分流：** 三種叫法都行——① 自然語言「去 AI 味」② `/humanizer`（語言路由入口，自動判中英）③ 直接指定 `/humanizer-tw`（中文：在地化、OpenCC 漏網）或 `/humanizer-en`（英文：37 patterns）。判語言看「要被改的文字」，不是你下指令的語言。
...
| [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop) | 英文編輯型 skill，voice-first、編輯品味佳但無量化評測、無中文層；其 3 個獨有 pattern 已中文化進 humanizer-tw v1.3.0 |
...
不只嘴上說。把 humanizer-tw 跟上面提到的幾支同類 skill 丟進同一份 126 筆語料對打（Opus 改寫、各跑 3 次取平均）：

| skill | 去 AI 味 recall ↑ | 誤殺合法台灣用語 ↓ |
|---|---|---|
| **humanizer-tw** | **95%** | **8%** |
| blader（英文版） | 96% | 29% |
| hong | 91% | 32% |
| academic（humanizer-zh-academic） | 94% | 38% |
| shyuan | 95% | 39% |
| kevintsai | 95% | 42% |
...
- **humanizer-tw** 原創於 [yelban/humanizer.tw](https://github.com/yelban/humanizer.tw)，參考 [blader/humanizer](https://github.com/blader/humanizer) 與 [hardikpandya/stop-slop](https://github.com/hardikpandya/stop-slop) 的設計；v1.3.0 的「多數人迷思開場／冒號揭曉／自問自答」與結尾金句紀律參考 [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop)（MIT），中文化並補防誤殺 guard
- **humanizer-en** 基底 [blader/humanizer](https://github.com/blader/humanizer)（© 2025 Siqi Chen，MIT）v2.9.1，自 v3.0.0 起獨立演進（§34–36 參考 [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop)，並回流 humanizer-tw 的紀律）。其 MIT 授權見 [`humanizer-en/LICENSE`](humanizer-en/LICENSE)
